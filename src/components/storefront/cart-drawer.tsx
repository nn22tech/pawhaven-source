...
export function CartDrawer({
  ...
  async function placeOrder(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.emailSent) {
        toast.success(
          "Order placed! We've received your order and will contact you shortly.",
        );
      } else if (data.mailto) {
        window.location.href = data.mailto;
        toast.success(
          "Order placed! Your email app should now open with the order details.",
        );
      } else {
            ...
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              ...
              <Field label="Location">
                <Input
                  value={cust.location}
                  onChange={(e) =>
                    setCust({ ...cust, location: e.target.value })
                  }
              ...
              <p className="rounded-md bg-primary/5 p-3 text-xs text-muted-foreground">
                ...
                Clicking <strong>Place Order</strong> will send your order
                directly to our store. We'll contact you to confirm!
              </p>
            </div>
            <SheetFooter className="border-t">
