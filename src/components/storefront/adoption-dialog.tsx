...
export function AdoptionDialog({ open, onOpenChange, prefill }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    petType: "", petColor: "", petAge: "", amount: "", experience: "", message: "",
  });
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!form.fullName || !form.email || !form.location || !form.petType) {
      toast.error("Please fill in your name, email, location and the pet type.");
      return;
    }
    setSubmitting(true);
    try {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Tell us about yourself and the pet you'd love to adopt. We'll review and get back to you.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={submit} className="grid gap-4 py-2">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Location" required>
              <Input value={form.location} onChange={set("location")} placeholder="City, Country" required />
            </Field>
          </div>

          <div className="rounded-lg border bg-muted/30 p-4">
            <h4 className="mb-3 text-sm font-semibold">Pet you're interested in</h4>
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Pet Type / Breed" required>
                <Input value={form.petType} onChange={set("petType")} placeholder="e.g. Golden Retriever" required />
              </Field>
              <Field label="Preferred Color">
                <Input value={form.petColor} onChange={set("petColor")} placeholder="e.g. Golden" />
              </Field>
              <Field label="Preferred Age">
                <Input value={form.petAge} onChange={set("petAge")} placeholder="e.g. Puppy (under 1 yr)" />
              </Field>
              <Field label="Budget (USD)">
                <Input type="number" min="0" value={form.amount} onChange={set("amount")} placeholder="e.g. 400" />
              </Field>
            </div>
          </div>
